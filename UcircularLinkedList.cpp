...
// main page for UI doubly linked list
UcircularLinkedList::UcircularLinkedList(wxPanel* parent) : wxPanel(parent) {
	...
	arrow_image = wxBitmap(wxT("assets/arrow3.png"), wxBITMAP_TYPE_PNG);
	arrow_image.Rescale(arrow_image, FromDIP(wxSize(13, 30)));
	for (short i = 2; i <= max_size + 1; ++i) {
		arrow3[i] = new wxStaticBitmap(base, wxID_ANY, arrow_image, wxPoint(box_position[i] - 25, box_y + 12));
		arrow3[i]->Hide();
	}

	arrow_image = wxBitmap(wxT("assets/arrow4.png"), wxBITMAP_TYPE_PNG);
	arrow_image.Rescale(arrow_image, FromDIP(wxSize(13, 30)));
	for (short i = 1; i <= max_size; ++i) {
		arrow4[i] = new wxStaticBitmap(base, wxID_ANY, arrow_image, wxPoint(box_position[i] - 13, box_y + 12));
		arrow4[i]->Hide();
	}
	...
	button_back->Bind(wxEVT_BUTTON, &UcircularLinkedList::goBack, this);
	button_create_random->Bind(wxEVT_BUTTON, &UcircularLinkedList::createRandom, this);
	button_import_file->Bind(wxEVT_BUTTON, &UcircularLinkedList::importFile, this);
	button_export_file->Bind(wxEVT_BUTTON, &UcircularLinkedList::exportFile, this);
	button_insert->Bind(wxEVT_BUTTON, &UcircularLinkedList::insertPosition, this);
	button_delete->Bind(wxEVT_BUTTON, &UcircularLinkedList::deletePosition, this);
	button_update->Bind(wxEVT_BUTTON, &UcircularLinkedList::updatePosition, this);
	button_access->Bind(wxEVT_BUTTON, &UcircularLinkedList::accessPosition, this);
	button_search->Bind(wxEVT_BUTTON, &UcircularLinkedList::searchValue, this);

	button_sbs->Bind(wxEVT_BUTTON, &UcircularLinkedList::sbsModeOn, this);
	button_sbs2->Bind(wxEVT_BUTTON, &UcircularLinkedList::sbsModeOff, this);
	button_next->Bind(wxEVT_BUTTON, &UcircularLinkedList::nextStep, this);
	button_skip->Bind(wxEVT_BUTTON, &UcircularLinkedList::skipStep, this);
...
// go back to menu
void UcircularLinkedList::goBack(wxCommandEvent& e) {
	wxCommandEvent empty_e = wxCommandEvent();
	sbsModeOff(empty_e);
	createRandom(empty_e);
...
// create random input
void UcircularLinkedList::createRandom(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	rClear(id_circular_linked_list, box, base);
	rCreateRandom(id_circular_linked_list, box, base);

	for (short i = 1; i <= max_size + 1; ++i) {
		if (i <= max_size && id_pos[i] != nullptr) {
			id_pos[i]->Destroy();
			id_pos[i] = nullptr;
		}
	...
	for (short i = 1; i <= rBoxSize(id_circular_linked_list); ++i) {
	...
	arrow3[rBoxSize(id_circular_linked_list) + 1]->Show();

	input_insert_pos->SetRange(0, rBoxSize(id_circular_linked_list));
	input_delete_pos->SetRange(0, rBoxSize(id_circular_linked_list) - 1);
	input_update_pos->SetRange(0, rBoxSize(id_circular_linked_list) - 1);
	input_access_pos->SetRange(0, rBoxSize(id_circular_linked_list) - 1);
...
void UcircularLinkedList::importFile(wxCommandEvent& e) {
	...
	if (file.is_open()) {
		...
		line = "," + line;
		rClear(id_circular_linked_list, box, base);
		rStringToBox(id_circular_linked_list, line, box, base);
		input_insert_pos->SetRange(0, rBoxSize(id_circular_linked_list) - 1);
	}
	else {
...
void UcircularLinkedList::exportFile(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}

	if (rBoxSize(id_circular_linked_list) == 0) {
	...
	for (short i = 0; i <= max_size; ++i) {
		if (rAtBox(id_circular_linked_list, i) == -1000) {
			break;
		}
		...
		export_file << rAtBox(id_circular_linked_list, i);
	}
...
void UcircularLinkedList::insertPosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		rInsert(id_circular_linked_list, pos, val, box, base);
		skip_sbs = false;
	}
	else {
		during_sbs = true;
		fix_pos = pos;
		fix_val = val;
		if (rBoxSize(id_circular_linked_list) == max_size) {
			during_sbs = false;
			showError("The maximum number of element you can have is 12");
			return;
		}
		else {
			rInsertSbs(id_circular_linked_list, pos, val, arrow);
		}
	}
...
void UcircularLinkedList::deletePosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		rDelete(id_circular_linked_list, pos, box, base);
		skip_sbs = false;
	}
	else {
		during_sbs = true;
		fix_pos = pos;
		rDeleteSbs(id_circular_linked_list, pos, arrow);
	}
...
void UcircularLinkedList::updatePosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		rUpdate(id_circular_linked_list, pos, val, box, base);
		skip_sbs = false;
	}
	else {
		during_sbs = true;
		fix_pos = pos;
		fix_val = val;
		rUpdateSbs(id_circular_linked_list, pos, val, arrow);
	}
}

// return the value at a specific position
void UcircularLinkedList::accessPosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		skip_sbs = false;
		wxString text = "";
		text << rAtBox(id_circular_linked_list, pos);
		...
		during_sbs = true;
		fix_pos = pos;
		rAccessSbs(id_circular_linked_list, pos, arrow);
	}
}

// search the position of first appearence of a specific value
void UcircularLinkedList::searchValue(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		skip_sbs = false;
		short pos = rSearch(id_circular_linked_list, val);
		wxString text = "";
		...
		during_sbs = true;
		fix_val = val;
		rSearchSbs(id_circular_linked_list, val, arrow);
	}
}

// turn on the step by step mode
void UcircularLinkedList::sbsModeOn(wxCommandEvent& e) {
	sbs_mode = true;
	skip_sbs = false;
	button_sbs->Hide();
	button_sbs2->Show();
...
void UcircularLinkedList::sbsModeOff(wxCommandEvent& e) {
	wxCommandEvent empty_e = wxCommandEvent();
	skipStep(empty_e);
...
void UcircularLinkedList::nextStep(wxCommandEvent& e) {
	if (during_sbs == false) {
		return;
	}

	if (rNext(id_circular_linked_list, box, base, arrow) == true) {
		wxCommandEvent empty_e = wxCommandEvent();
		skipStep(empty_e);
		during_sbs = false;
	}
...
void UcircularLinkedList::skipStep(wxCommandEvent& e) {
	...
	switch (rSbsMode(id_circular_linked_list)) {
	case 1:
	...
	case 4:
		text << rAtBox(id_circular_linked_list, fix_pos);
		output_access_val = new wxStaticText(base, wxID_ANY, text, wxPoint(775, 475));
		output_access_val->Show();
		skip_sbs = false;
		break;
	...
	case 5:
		pos = rSearch(id_circular_linked_list, fix_val);
		skip_sbs = false;
