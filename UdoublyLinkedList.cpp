...
UdoublyLinkedList::UdoublyLinkedList(wxPanel* parent) : wxPanel(parent) {
	...
	button_back->Bind(wxEVT_BUTTON, &UdoublyLinkedList::goBack, this);
	button_create_random->Bind(wxEVT_BUTTON, &UdoublyLinkedList::createRandom, this);
	button_import_file->Bind(wxEVT_BUTTON, &UdoublyLinkedList::importFile, this);
	button_export_file->Bind(wxEVT_BUTTON, &UdoublyLinkedList::exportFile, this);
	button_insert->Bind(wxEVT_BUTTON, &UdoublyLinkedList::insertPosition, this);
	button_delete->Bind(wxEVT_BUTTON, &UdoublyLinkedList::deletePosition, this);
	button_update->Bind(wxEVT_BUTTON, &UdoublyLinkedList::updatePosition, this);
	button_access->Bind(wxEVT_BUTTON, &UdoublyLinkedList::accessPosition, this);
	button_search->Bind(wxEVT_BUTTON, &UdoublyLinkedList::searchValue, this);

	button_sbs->Bind(wxEVT_BUTTON, &UdoublyLinkedList::sbsModeOn, this);
	button_sbs2->Bind(wxEVT_BUTTON, &UdoublyLinkedList::sbsModeOff, this);
	button_next->Bind(wxEVT_BUTTON, &UdoublyLinkedList::nextStep, this);
	button_skip->Bind(wxEVT_BUTTON, &UdoublyLinkedList::skipStep, this);
...
// go back to menu
void UdoublyLinkedList::goBack(wxCommandEvent& e) {
	wxCommandEvent empty_e = wxCommandEvent();
	sbsModeOff(empty_e);
	createRandom(empty_e);
...
// create random input
void UdoublyLinkedList::createRandom(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	rClear(id_doubly_linked_list, box, base);
	rCreateRandom(id_doubly_linked_list, box, base);
	...
	for (short i = 1; i <= rBoxSize(id_doubly_linked_list); ++i) {
	...
	input_insert_pos->SetRange(0, rBoxSize(id_doubly_linked_list));
	input_delete_pos->SetRange(0, rBoxSize(id_doubly_linked_list) - 1);
	input_update_pos->SetRange(0, rBoxSize(id_doubly_linked_list) - 1);
	input_access_pos->SetRange(0, rBoxSize(id_doubly_linked_list) - 1);
...
void UdoublyLinkedList::importFile(wxCommandEvent& e) {
	...
	if (file.is_open()) {
		...
		line = "," + line;
		rClear(id_doubly_linked_list, box, base);
		rStringToBox(id_doubly_linked_list, line, box, base);
		input_insert_pos->SetRange(0, rBoxSize(id_doubly_linked_list) - 1);
	}
	else {
...
void UdoublyLinkedList::exportFile(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}

	if (rBoxSize(id_doubly_linked_list) == 0) {
...
void UdoublyLinkedList::insertPosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		rInsert(id_doubly_linked_list, pos, val, box, base);
		skip_sbs = false;
	}
	else {
		during_sbs = true;
		fix_pos = pos;
		fix_val = val;
		if (rBoxSize(id_doubly_linked_list) == max_size) {
			during_sbs = false;
			showError("The maximum number of element you can have is 12");
			return;
		}
		else {
			rInsertSbs(id_doubly_linked_list, pos, val, arrow);
		}
	}
...
void UdoublyLinkedList::deletePosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		rDelete(id_doubly_linked_list, pos, box, base);
		skip_sbs = false;
	}
	else {
		during_sbs = true;
		fix_pos = pos;
		rDeleteSbs(id_doubly_linked_list, pos, arrow);
	}
...
void UdoublyLinkedList::updatePosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		rUpdate(id_doubly_linked_list, pos, val, box, base);
		skip_sbs = false;
	}
	else {
		during_sbs = true;
		fix_pos = pos;
		fix_val = val;
		rUpdateSbs(id_doubly_linked_list, pos, val, arrow);
	}
}

// return the value at a specific position
void UdoublyLinkedList::accessPosition(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		skip_sbs = false;
		wxString text = "";
		text << rAtBox(id_doubly_linked_list, pos);
		...
		during_sbs = true;
		fix_pos = pos;
		rAccessSbs(id_doubly_linked_list, pos, arrow);
	}
}

// search the position of first appearence of a specific value
void UdoublyLinkedList::searchValue(wxCommandEvent& e) {
	if (during_sbs == true) {
		return;
	}
	...
	if (sbs_mode == false || skip_sbs == true) {
		skip_sbs = false;
		short pos = rSearch(id_doubly_linked_list, val);
		wxString text = "";
		...
		during_sbs = true;
		fix_val = val;
		rSearchSbs(id_doubly_linked_list, val, arrow);
	}
}

// turn on the step by step mode
void UdoublyLinkedList::sbsModeOn(wxCommandEvent& e) {
	sbs_mode = true;
	skip_sbs = false;
	button_sbs->Hide();
	button_sbs2->Show();
...
void UdoublyLinkedList::sbsModeOff(wxCommandEvent& e) {
	wxCommandEvent empty_e = wxCommandEvent();
	skipStep(empty_e);
...
void UdoublyLinkedList::nextStep(wxCommandEvent& e) {
	if (during_sbs == false) {
		return;
	}

	if (rNext(id_doubly_linked_list, box, base, arrow) == true) {
		wxCommandEvent empty_e = wxCommandEvent();
		skipStep(empty_e);
		during_sbs = false;
	}
...
void UdoublyLinkedList::skipStep(wxCommandEvent& e) {
	if (during_sbs == false) {
		return;
	}
	...
	switch (rSbsMode(id_doubly_linked_list)) {
	case 1:
	...
	case 4:
		text << rAtBox(id_doubly_linked_list, fix_pos);
		output_access_val = new wxStaticText(base, wxID_ANY, text, wxPoint(775, 475));
		output_access_val->Show();
		skip_sbs = false;
		break;
	...
	case 5:
		pos = rSearch(id_doubly_linked_list, fix_val);
		skip_sbs = false;
